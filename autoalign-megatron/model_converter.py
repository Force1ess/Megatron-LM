...
# this is for plugin import in convert.py
sys.path.append(os.path.join(os.environ['MEGATRON_PATH'], "tools", "checkpoint"))

from tools.checkpoint import loader_llama_mistral as loader
from tools.checkpoint import saver_mcore as saver
from tools.checkpoint.saver_mcore import get_model_schema, PkgVersion, version, torch
from tools.checkpoint.loader_llama_mistral import (
    add_arguments as add_arguments_original,
    load_args_from_checkpoint as load_args_from_checkpoint_original,
)



def load_args_from_checkpoint(args):
    load_args_from_checkpoint_original(args)
...
def add_arguments(parser):
    add_arguments_original(parser)
    parser._option_string_actions['--model-size'].choices = None


def set_postprocess_state(args, model, hf_model):
    '''Set output layer & norm params.'''
    model.language_model.encoder.final_norm.weight.data.copy_(hf_model.model.norm.weight)
    if args.untie_embeddings_and_output_weights:  # add untie branch
        model.language_model.output_layer.weight.data.copy_(hf_model.lm_head.weight)

# this can be deleted if my PR accepted: https://github.com/NVIDIA/Megatron-LM/pull/1351
...
def save_checkpoint(queue, args):
    ...
    schema = get_model_schema(
        md.model_type, margs.transformer_impl, margs.num_experts, margs.expert_model_parallel_size
    )
    ...
    print("Done!")
saver.save_checkpoint = save_checkpoint

# monkey patch the loader
loader.add_arguments = add_arguments
loader.set_postprocess_state = set_postprocess_state
loader.load_args_from_checkpoint = load_args_from_checkpoint
sys.modules["loader_llama_mistral"] = loader
sys.modules["saver_mcore"] = saver

if __name__ == '__main__':
    from tools.checkpoint.convert import main
